    ...
    def initialize_indicator(self, indicator_name, params):
        ...
        try:
            ...
            # Momentum Indicators
                ...
                self.indicators[f'macd_{fast_period}_{slow_period}_{signal_period}'] = self.I(lambda: macd)
                self.indicators[f'macdsignal_{fast_period}_{slow_period}_{signal_period}'] = self.I(lambda: signal)
                
            elif indicator_name == 'stoch':
                k_period = int(params[0])
                d_period = int(params[1])
                slowk, slowd = talib.STOCH(high, low, close, fastk_period=k_period, slowk_period=d_period)
...
class BacktestingAgent:
    ...
    def get_latest_algorithm(self, company_name):
        ...
        pattern = str(self.output_dir / 'logs' / f"{company_name}_algorithm-*.json")
